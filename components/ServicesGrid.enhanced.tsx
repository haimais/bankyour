...
import { getServiceTranslation } from "@/lib/i18n/serviceTranslations";
import { ServiceType, LanguageCode } from "@/lib/types";
...
export function ServicesGrid({ onSelectService }: ServicesGridProps) {
  ...
  const localeKey = (locale === "ru" ? "ru" : "en") as "ru" | "en";
    ...
    <section id="services" className="mx-auto w-full max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {SERVICE_CATEGORIES.map((service, index) => {
          // Get full translated title for the service
          const translatedTitle = getServiceTranslation(service.key, locale as LanguageCode);
          
          // Fallback to provided translations
          const title = translatedTitle || service.title[localeKey];
          const description = service.description[localeKey];
